...
<Remark >
As observed earlier and above, the sequence of partial sums $\left(\sum_{k=-N}^{N} e^{-i2\pi kPt}\right)$ converges to a periodic Dirac delta function as a distribution (see also Theorem). This sequence is known as the Dirichlet kernel sequence.
</Remark>
...
It also follows that if $T > \frac{1}{2B}$, the exact reconstruction will not be possible since the shifted frequency components will overlap with each other. This is known as **aliasing**. When there is aliasing, one can compute the error in the reconstruction via Parseval's theorem (see Theorem).
...
b) By the relevant section, we have that $\hat{\overline{w_P}}(\phi) = \overline{w_P}(\hat{\phi})$. Then,
...
Using the geometric series formula and the fact that $\frac{\sin(2\pi(N+1/2)Pt)}{\sin(\pi Pt)}$ is periodic with period $1/P$, and for $t \in [-\frac{1}{2P}, \frac{1}{2P}]$ this function converges (as a distribution) to a delta function (see Theorem), the result follows:
