...
<Proof>
Let $\hat{\phi} = \mathcal{F}_{CC}(\phi)$. We first show that $\hat{\phi} \in \mathcal{S}$. We use Theorem in the following. We have
...
<Proof>
Recall from Exercise that Theorem (Riemann-Lebesgue Lemma) was utilized to arrive at this result. The key idea is as follows.
...
For the second term, define $g(x) = \frac{\phi(x) - \phi(0)}{\pi x}$. Since $\phi \in \mathcal{S}$, $g$ is smooth near $x = 0$ (with $g(0) = \frac{\phi'(0)}{\pi}$) and decays rapidly as $|x| \to \infty$. In particular, $g \in L_1(\mathbb{R})$. By the Riemann-Lebesgue lemma (Theorem):
