...
This appendix presents Cauchy's integral formula and the residue theorem, which are the complex analysis results underpinning the Nyquist stability criterion (Section 8.4) and the computation of inverse Laplace and Z-transforms via contour integration (Section 7.3).

---

## B.1 Cauchy's Integral Formula

<Theorem id="B.0.1" title="Cauchy's Integral Theorem">
...
<Theorem id="B.0.2" title="Cauchy's Integral Formula">
...
<Definition id="B.2.1" title="Residue">
...
<Theorem id="B.2.1" title="Residue Theorem">
...
<Remark>
**Intuition:** The residue theorem generalizes Cauchy's integral theorem to functions with poles: the integral around a closed contour equals $2\pi i$ times the sum of the residues at the poles enclosed by the contour. This is the key tool for computing inverse Laplace transforms and is the mathematical engine behind the Nyquist criterion -- the winding number in the principle of variation of the argument (Theorem 8.4.2) is computed via residues.
</Remark>
...
The principle of variation of the argument (Theorem 8.4.2), which is the foundation of the Nyquist stability criterion, is a direct consequence of the residue theorem. For a function $f$ with $Z$ zeros and $P$ poles inside a contour $\Gamma$:
