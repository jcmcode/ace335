...
$$|x_n(t) - x(t)| \leq \epsilon, \qquad \text{}$$
...
$$x_n(t) = \begin{cases} 1, & \text{if } 0 \leq t \leq 1/2 \\ -2^n(t - 1/2) + 1 & \text{if } 1/2 < t < (1/2) + (1/2)^n \\ 0, & \text{if } (1/2) + (1/2)^n \leq t \leq 1 \end{cases} \qquad \text{}$$
...
$$x = \begin{cases} 1, & \text{if } 0 \leq t \leq 1/2 \\ 0, & \text{if } (1/2) < t \leq 1 \end{cases} \qquad \text{}$$
...
<Proof>
**Part (i):** We first show that if $x - m_0$ is not orthogonal to $B$, then $m_0$ cannot be optimal. Suppose that $m_0 \in B$ is such that $\exists m \in B$ with $\langle x - m_0, m \rangle > 0$. Without any loss, take $\|m\| = 1$ and $\langle x - m_0, m \rangle = \delta$. We can show that with $m_1 = m_0 + \delta m$, we will have $\|x - m_1\|^2 = \|x - m_0\|^2 - \delta^2 < \|x - m_0\|^2$, and thus $m_0$ cannot be a minimizer of .
...
$$\delta = \inf_{m \in M} \|x - m\| \qquad \text{}$$
...
$$= 2\langle x - m_k, x - m_k \rangle + 2\langle x - m_n, x - m_n \rangle \qquad \text{}$$

Write

$$\langle x - m_k + x - m_n, x - m_k + x - m_n \rangle = \langle 2(x - \frac{m_k + m_n}{2}), 2(x - \frac{m_k + m_n}{2}) \rangle$$
$$= 4\langle x - \frac{m_k + m_n}{2}, x - \frac{m_k + m_n}{2} \rangle \qquad \text{}$$

Since $\frac{m_k + m_n}{2} \in B$, by ,
...
We claim that the difference goes to zero; this follows from the continuity of inner product (Theorem). Indeed,
...
$$\leq \|x - m_n\|\|m_n - \tilde{m}\| + \|m_n - \tilde{m}\|\|x - \tilde{m}\| \qquad \text{}$$
...
Consider the sequence of vectors $x_n := \sum_{i=1}^{n} \langle x, e_i \rangle e_i$. Observe that $x_n$ converges to a limit by Theorem. To see that the sequence $\sum_{i=1}^{\infty} |\langle x, e_i \rangle|^2$ is summable, use orthonormality and apply the Pythagorean theorem to conclude that $\sum_{i=1}^{\infty} |\langle x, e_i \rangle|^2 \leq \|x\|^2$. Therefore, the limit $\sum_{i=1}^{\infty} \langle x, e_i \rangle e_i$ is well-defined.
...
$$= \langle x, e_m \rangle - \langle x, e_m \rangle = 0. \qquad \text{}$$
...
See Exercise for a probability theoretic proof, by noting that if $X_k$ is a collection of $\{0, 1\}$ valued independent and identically distributed random variables with $P(X_k = 1) = t$, we have that
...
<Proof>
Given Theorem, we can show that polynomials $\{t^k, k \in \mathbb{Z}_+\}$ defined on $[0, 1]$ can be used to construct a complete orthogonal sequence. In view of Theorem, this will imply separability.
...
$$\langle h, h \rangle = \langle h - \sum_{i=1}^{n} \alpha_i t^k, h \rangle \leq \|h - \sum_{i=1}^{n} \alpha_i t^k\| \|h\| \qquad \text{}$$

The expression $\|h - \sum_{i=1}^{n} \alpha_i t^k\|$ can be made arbitrarily small by first approximating $h$ with a continuous function with a given approximation error (by Theorem) and then approximating that fixed continuous function with a polynomial of some finite degree with another arbitrarily small approximation error (by Theorem). Thus $\|h\|$ must be less than any positive real number. Therefore, there exists a complete sequence. This sequence can be made orthonormal by the Gram-Schmidt procedure.

Thus, Bernstein polynomials are dense in $C([0, 1])$, but these are not orthogonal in $L_2([0, 1]; \mathbb{R})$; and if we apply the Gram-Schmidt procedure to $\{1, t, t^2, \cdots\}$, we can obtain an orthonormal collection of polynomials that is a complete sequence in $L_2([0, 1]; \mathbb{R})$ as we saw in the proof of Theorem. These orthonormal polynomials are called **Legendre polynomials**.
