...
<Proof>
**(i)** The controllable subspace is $\text{Range}(\mathcal{C}) = \text{Range}(\begin{bmatrix} B & AB & \cdots & A^{n-1}B \end{bmatrix})$. If $v$ is in this subspace, then $v = \sum_{k=0}^{n-1} A^k B \alpha_k$ for some coefficients $\alpha_k$. Then $Av = \sum_{k=0}^{n-1} A^{k+1} B \alpha_k = \sum_{k=1}^{n} A^k B \alpha_{k-1}$. By the Cayley-Hamilton theorem, $A^n B$ can be written as a linear combination of $\{B, AB, \ldots, A^{n-1}B\}$. Therefore $Av \in \text{Range}(\mathcal{C})$.

**(ii)** The unobservable subspace is $\text{Null}(\mathcal{O}) = \{v : CA^k v = 0, \; 0 \leq k \leq n-1\}$. If $v$ is in this subspace, then $CA^k(Av) = CA^{k+1}v = 0$ for $0 \leq k \leq n-2$. For $k = n-1$, $CA^{n-1}(Av) = CA^n v$. By Cayley-Hamilton, $A^n$ is a linear combination of $\{I, A, \ldots, A^{n-1}\}$, so $CA^n v$ is a linear combination of $\{Cv, CAv, \ldots, CA^{n-1}v\}$, all of which are zero. Therefore $Av \in \text{Null}(\mathcal{O})$.
</Proof>
